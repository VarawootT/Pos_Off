<style>
    .h1{     /* กำหนดให้ font-size 27 px */
        font-size: 18px;
    }
    .b1 {    /* กำหนดให้ font-weight ให้ตัวหน่า */
        font-weight: bold;
    }
    .font16{ /* กำหนดให้ font-size 18 px */
        font-size: 12px;
    }
    .p1 { /* กำหนดให้ font เป๋น arial */
        font-family: 'Arial Unicode MS', monospace;
    }
    .font20{ /* กำหนดให้ font-size 18 */
        font-size: 16px;
    }
</style>
...
@$result = $mysqli->query("SELECT wh,sta_vat, name,lname FROM `user` WHERE id='$userid'");    //กำหนดตัวแปล $result เพื่อรับ query ของ select * from user เพื่อคนหา user นี้
...
@$user_sql = $mysqli->query("SELECT  wh,name,lname  FROM `user` WHERE id='$SEN_USER_SALE'");  //ค้นหา user ที่ทำการขายที่เราต้องการเลือกในหน้า frm_report_summary และมาเป็นในตัวแปลของ $SEN_USER_SALE
@$row_user = $user_sql->fetch_assoc();                                                        //กำหนดให้ $row ให้ $result ดึงแถวผลลัพธ์เป็นอาเรย์ที่เชื่อมโยง
$user_name_select  = $row_user['name'] ?? '';                                                 //กำหนด user_name_select = $row_user ของแถว name
$user_lname_select = $row_user['lname'] ?? '';                                                //กำหนด user_lname_select = $row_user ของแถว lname
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------- ตัวเปิด mysql 
@$resultwh = $mysqli->query("SELECT name_bill FROM `p_wh` WHERE wh = '$user_wh'");            //กำหนด query สำหรับ select name_bill หรือ สถานที่ของเครื่อง                                                  
...
b.so,                                                                           /* select หา so เพื่อไว้ GROUP BY และทดสอบ ว่าค่าที่ได้นั้นถูก */  
(b.receive + b.discount) AS receiveall,                                         /* select หา กำหนดให้ เงินที่รับ + ส่วนลด จะได้เงินจำนวนจริงออกมาได้ */ 
(b.discount) AS discount                                                        /* select หา ส่วนลด */ 
...
<tbody> <!-- เปิดเนื่อหาหัวตาราง -->
    ...
    <!-- ------------------------------------------------------------------------วันที่เปิด  -->
    <tr>
        <td colspan="5" align="left" class="p1 font16">วันที่เวลา/เปิดการขาย  :  <?php if($date_start_open != ""){ echo $date_start_open ; } else{ echo "ยังไม่มีเวลาเปิดร้าน";
        } ?> </td> <!-- ชิดซ้าย รวม 5 แถว -->
    </tr>

    <!-- ------------------------------------------------------------------------วันที่ปิด -->
    <tr>
        <td colspan="5" align="left" class="p1 font16">วันที่เวลา/ปิดการขาย  :  <?php if($date_end_close != ""){ echo $date_end_close; }else{ echo "ยังไม่มีเวลาปิดร้าน"; }   ?></td> <!-- ชิดซ้าย รวม 5 แถว -->
    </tr>
    ...
    <tr>
        ...
        <td colspan="2" align="right" class="p1 font16"><?php if($SUMRECEIVESTAC1 != 0){echo number_format(-$SUMRECEIVESTAC1,2);}else{echo "0.00";}  ?></td> <!-- ชิดขวา รวม 2 แถว -->
    </tr>
    <!-- ------------------------------------------------------------------------ส่วนลดทั้งหมด -->
    <tr>
    ...
    <!-- ------------------------------------------------------------------------รวมทั้งหมด -->
    <tr>
        <td colspan="3" align="left"  class="p1 font16">รวมทั้งหมด</td> <!-- ชิดซ้าย รวม 3 แถว -->
        <td colspan="2" align="right" class="p1 font16"><?php echo number_format($NETSALES,2) ?></td> <!-- ชิดขวา รวม 2 แถว -->
    </tr>
    <!-- ------------------------------------------------------------------------ -->
